...
class Polynomial:
    ...
    def __mul__(self, other):
        ...
        for i in range(len(self.coefficients)):
            ...
            for j in range(len(other.coefficients)):
                buf[i +
                    j] = buf[i +
                             j] + self.coefficients[i] * other.coefficients[j]
    ...
    def __eq__(self, other):
        ...
        if self.degree() == -1:
            return True
        return all(self.coefficients[i] == other.coefficients[i]
                   for i in range(len(self.coefficients)))
    ...
    def divide(numerator, denominator):
        ...
        for i in range(numerator.degree() - denominator.degree() + 1):
            ...
            subtractee = Polynomial([field.zero()] * shift +
    ...
    def interpolate_domain(domain, values):
        ...
        for i in range(len(domain)):
            ...
            for j in range(len(domain)):
                if j == i:
                    continue
                prod = prod * (x - Polynomial([domain[j]])) * Polynomial(
                    [(domain[i] - domain[j]).inverse()])
            acc = acc + prod
        return acc
    ...
    def scale(self, factor):
        return Polynomial([(factor ^ i) * self.coefficients[i]
                           for i in range(len(self.coefficients))])

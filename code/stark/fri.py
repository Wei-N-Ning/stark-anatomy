...
class Fri:
    ...
    def __init__(self, offset, omega, initial_domain_length, expansion_factor,
        ...
        assert (self.num_rounds() >=
                1), "cannot do FRI with less than one round"
        ...
        for s in range(self.num_colinearity_tests):
            proof_stream.push(
                (current_codeword[a_indices[s]],
                 current_codeword[b_indices[s]], next_codeword[c_indices[s]]))
    ...
    def prove(self, codeword, proof_stream):
        ...
        top_level_indices = self.sample_indices(
            proof_stream.prover_fiat_shamir(), len(codewords[1]),
            len(codewords[-1]), self.num_colinearity_tests)
    ...
    def verify(self, proof_stream, polynomial_values):
        ...
        for r in range(0, self.num_rounds() - 1):
            ...
            a_indices = [index for index in c_indices]
            b_indices = [
                index + (self.domain_length >> (r + 1)) for index in a_indices
            ]

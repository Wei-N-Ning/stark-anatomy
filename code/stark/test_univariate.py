...
def test_interpolate():
    ...
    for i in range(len(domain)):
        assert (poly.evaluate(
            domain[i]) == values[i]), "fail interpolate test 1"

    # evaluation in random point is nonzero with high probability
    assert (poly.evaluate(FieldElement(363, field)) !=
            zero), "fail interpolate test 2"
...
def test_zerofier():
    ...
    for trial in range(0, 100):
        ...
        for d in domain:
            assert (zerofier.evaluate(d) == field.zero()
        ...
        assert (zerofier.evaluate(random) !=
                field.zero()), "zerofier evaluates to zero where it should not"

...
def test_rescue_prime():
    ...
    assert (rp.hash(FieldElement(1, rp.field)) == FieldElement(
        244180265933090377212304188905974087294,
        rp.field)), "rescue prime test vector 1 failed"
    assert (rp.hash(
    ...
    trace = rp.trace(a)
    assert (trace[0][0] == a and trace[-1][0]
            == b), "rescue prime trace does not satisfy boundary conditions"
...
def test_trace():
    ...
    input_element = FieldElement(57322816861100832358702415967512842988,
                                 rp.field)
    b = FieldElement(89633745865384635541695204788332415101, rp.field)
    ...
    for condition in rp.boundary_constraints(output_element):
        ...
        if trace[cycle][element] != value:
            print("rescue prime boundary condition error: trace element",
                  element, "at cycle", cycle, "has value",
                  trace[cycle][element], "but should have value", value)
            assert (False)
    ...
    for k in range(10):
        ...
        if k == 0:
            register_index = 1
            cycle_index = 22
            value_ = FieldElement(17274817952119230544216945715808633996,
                                  rp.field)

        # perturb
        trace[cycle_index][
            register_index] = trace[cycle_index][register_index] + value_

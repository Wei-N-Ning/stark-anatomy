...
class FastRPSSS:
    ...
    def __init__(self):
        ...
        self.stark = FastStark(self.field,
                               expansion_factor,
    ...
    def stark_prove(self, input_element, proof_stream):
        ...
        transition_constraints = self.rp.transition_constraints(
            self.stark.omicron)
        boundary_constraints = self.rp.boundary_constraints(output_element)
        proof = self.stark.prove(trace, transition_constraints,
    ...
    def stark_verify(self, output_element, stark_proof, proof_stream):
        ...
        transition_constraints = self.rp.transition_constraints(
            self.stark.omicron)
        return self.stark.verify(stark_proof, transition_constraints,
                                 boundary_constraints,
                                 self.transition_zerofier_root, proof_stream)

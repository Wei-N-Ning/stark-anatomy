...
from stark.algebra import Field, FieldElement
from stark.fri import Fri
from stark.ip import ProofStream
from stark.univariate import Polynomial
...
class TestFRI(unittest.TestCase):
    def test_fri(self):
        ...
        assert (omega ^
                (1 <<
                 (log_codeword_length - 1)) != field.one()), "omega not primitive"

        fri = Fri(generator, omega, initial_codeword_length, expansion_factor,
                  num_colinearity_tests)

        polynomial = Polynomial(
            [FieldElement(i, field) for i in range(degree + 1)])

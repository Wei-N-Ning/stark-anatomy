...
class FastStark:
    ...
    def __init__(self,
        ...
        assert (len(bin(field.p)) - 2 >= security_level
                ), "p must have at least as many bits as security level"
        assert (expansion_factor & (expansion_factor - 1) == 0
        ...
        self.omicron_domain_length = 1 << len(
            bin(self.randomized_trace_length *
                transition_constraints_degree)[2:])
    ...
    def preprocess(self):
        transition_zerofier = fast_zerofier(
            ...
            self.omicron, len(self.omicron_domain))
        transition_zerofier_codeword = fast_coset_evaluate(
            transition_zerofier, self.generator, self.omega,
    ...
    def boundary_quotient_degree_bounds(self, randomized_trace_length,
                                        boundary):
    ...
    def prove(self,
        ...
        for s in range(self.num_registers):
            ...
            trace_polynomials = trace_polynomials + [
                fast_interpolate(trace_domain, single_trace, self.omicron,
                                 self.omicron_domain_length)
            ]
        ...
        for s in range(self.num_registers):
            boundary_quotient_codewords = boundary_quotient_codewords + [
                fast_coset_evaluate(boundary_quotients[s], self.generator,
                                    self.omega, self.fri_domain_length)
            ]
        ...
        transition_quotients = [
            fast_coset_divide(tp, transition_zerofier, self.generator,
        ...
        randomizer_codeword = fast_coset_evaluate(randomizer_polynomial,
                                                  self.generator, self.omega,
        ...
        for i in range(len(transition_quotients)):
            ...
            shift = max_degree - self.transition_quotient_degree_bounds(
                transition_constraints)[i]
            terms += [(x ^ shift) * transition_quotients[i]]
        for i in range(self.num_registers):
            terms += [boundary_quotients[i]]
            shift = max_degree - self.boundary_quotient_degree_bounds(
                len(trace), boundary)[i]
        ...
        combined_codeword = fast_coset_evaluate(combination, self.generator,
                                                self.omega,
                                                self.fri_domain_length)
    ...
    def verify(self,
        ...
        # get weights for nonlinear combination
        weights = self.sample_weights(
            1 + 2 * len(transition_constraints) +
        ...
        for i in range(len(indices)):
            ...
            for s in range(self.num_registers):
                ...
                shift = self.max_degree(
                    ...
                    randomized_trace_length, boundary)[s]
